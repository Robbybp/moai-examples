# A MathOptAI-MadNLP linear solver plugin

We have prototyped a MadNLP linear solver that exploits the block-triangular structure
implied by a neural network represented as a MathOptAI `PipelineFormulation`.
...
These are generated offline with the `train-mnist.py` Python script.
...
```
python train-mnist.py --nodes=128 --layers=4 --activation=relu
python train-mnist.py --nodes=512 --layers=4 --activation=relu
python train-mnist.py --nodes=1024 --layers=4 --activation=relu
python train-mnist.py --nodes=2048 --layers=4 --activation=relu
```
...
a hard-coded local path to my `HSL_jll`. `HSL_jll` is very important to
reproduce these results. Maybe this works:
```
julia --project=.
...
```
julia --project=. madnlp-runtime-sweep.jl
# TODO: commands to reproduce _all_ the results.
```

## Profiling (mostly for my future reference!)
